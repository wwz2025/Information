<template>
	<view class="rf-floor-index">
		<!--广告图-->
		<!-- <view class="banner" v-if="bannerShow" @tap="toBanner(banner)" >
			<image :src="banner.pic" mode="aspectFill" />
		</view> -->
		<!--列表-->
		...
		<rf-recom-product-list :bottom="bottom" :list="list.length > 0 ? list : [0, 0]"></rf-recom-product-list>
	</view>
</template>
<script>/**
 *@des 首页楼层组件
...
import rfRecomProductList from '@/components/rf-recom-product-list';
export default {
	name: 'rfFloorIndex',
	components: { rfRecomProductList },
	props: {
		// 列表
		list: {
			type: Array,
