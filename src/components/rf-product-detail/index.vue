		...
		<view class="detail" v-if="product.sku.name">
			...
			<view class="carousel">
				<swiper indicator-dots circular="true" duration="400" controls touchable>
					...
					<swiper-item class="swiper-item" v-for="(item, index) in product.images" :key="index">
						...
						<u-tag class="content"  plain shape="circle" size="mini" :text="`${index + 1}  / ${product.images.length}`"></u-tag>
					</swiper-item>
				</swiper>
			</view>
			<!--商品信息-->
			<view class="introduce-section">
				...
				<view class="introduce-second-line">
					...
					<view class="share">
						<u-tag type="info" size="mini" tui-tag-class="tui-tag-share tui-size" shape="circle">
				...
				<view class="eva-box" v-if="commentList && commentList.length > 0">
					...
					<view class="right">
						<view class="name">
							...
							<u-rate v-if="evaluateList.length > 0" size="16" disabled="true" :value="evaluateList[0].score" :activeColor="themeColor.color" />
			...
			<view class="detail-desc">
				...
				<u-parse lazy-load :content="product.sku.mobileDesc"></u-parse>
			...
			<view class="page-bottom">
				<view class="page-bottom-bth-wrapper">
					...
					<navigator url="/pages/cart/cart" open-type="switchTab" class="p-b-btn cart">
						...
						<text>购物车</text>
						<u-badge v-if="hasLogin && cartNum && cartNum > 0" type="error"  :value="cartNum"></u-badge>
					</navigator>
					<!-- #ifdef MP-WEIXIN -->
