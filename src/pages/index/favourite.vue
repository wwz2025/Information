			...
			>
				<swiper-item v-for="tabItem in tabBars" :key="tabItem.id">
					<scroll-view
						...
						<view v-for="(item, index) in tabItem.newsList" :key="index" class="news-item" @click="navToCmsDetails(item)"  v-if="tabItem.newsId=='report'">
							<text class="title"  @click="reportClick(item.id)">
								类型：{{item.reportTypeName}}
							</text>
							<text class="title" @click="reportClick(item.id)">理由：{{item.reportDesc}}</text>
							<view class="media-item-line" ></view>
						</view>
	...
	export default {
		...
		async changeTab(e) {
				...
				let tabItem = this.tabBars[this.tabCurrentIndex];
					...
					// if(that.tabCurrentIndex !== that.tabQueryCurrentIndex && tabItem.loaded !== true){
			...
			reportClick(id){
				uni.navigateTo({url:"/pages/user/report/detail?id="+id})
			},
		}
	}
</script>
