	...
	<view class="container">
		<view class="content">
			<view class="detail">
				<view class="comment">
					<view class="item">
						<view class="avatar"><image :src="info.comment.avatar"></image></view>
						<view class="comment-info">
							<view class="nickname-like">
								<view class="nickname">{{ info.comment.userName }}</view>
								<view class="like">
									...
									<view class="like-count">{{ info.comment.upVote }}</view>
								</view>
							</view>
							<view class="comment-desc">{{ info.comment.content }}</view>
							<view class="date-reply">
								<view class="date">{{ info.comment.createTime }}</view>
								<view @tap="initAddReply('commit', info.comment.id, info.comment.userName)" class="reply">回复TA</view>
							</view>
						</view>
					</view>
				</view>
				<view class="comment replys">
					<view class="title">
						全部回复
						<text v-if="info.reply.rows > 0">({{ info.reply.rows }})</text>
					</view>
						...
						<view class="comment-info">
							<view class="nickname-like">
								...
								<view class="like">
									...
									<view class="like-count">{{ item.upVote }}</view>
								</view>
							</view>
							<view class="comment-desc">
								<text>{{ item.pid > 0 ? '回复@' + item.userName + '：' : '' }}</text>
								{{ item.content }}
							</view>
							<view class="reply-count" v-if="item.reply_count > 0">全部{{ item.replyNum }}条回复 ></view>
							<view class="date-reply">
					...
					<u-loadmore  status="loading" v-if="hasMoreData"></u-loadmore>
					<view class="no-comment" v-if="this.info.comment.replyNum == 0">暂无回复，快来抢占沙发吧~</view>
				</view>
			</view>
			<view class="float-empty"></view>
			<view class="bg-masker" v-if="showAddReply" @tap="showAddReplyBg(false)"></view>
			<view class="add-comment" v-if="showAddReply">
				<view class="info">
					<view class="input">
						<textarea
							:cursor-spacing="20"
							...
							:placeholder="replyPid > 0 ? '回复 ' + replyUserName + '：' : '回复 ' + info.comment.userName + '：'"
						/>
					</view>
...
export default {
	...
	data() {
		return {
			info: {comment:{id:-1234},reply: { rows: 0, list: [] } },
			showAddReply: false,
			addReplyFocus: false,
			operate: '',
			commentContent: '',
	...
	onLoad(e) {
		if (e.comment) {
			this.info.comment = JSON.parse(e.comment);
			this.replyPid = this.info.comment.id;
		}
		if (e.operate == 'reply') {
			this.operate = e.operate;
		}
		console.log('----------infoComment:',this.info.comment );
		this.getReply();
	},
	onPullDownRefresh() {
		uni.showLoading({
	...
	methods: {
		...
		async getCommentDetail() {
				await this.$http.get(`${commentListUrl}`,{
					id:this.info.comment.id
				})
					.then(async res => {
						if (res.list) {
							this.info.comment = res;
							console.log('comment:',this.info.list );
							//this.getReply();
		...
		commentLike() {
			this.$app.request({
				...
				data: {
					comment_id: this.info.comment.id
				},
				method: 'POST',
				dataType: 'json',
				success: res => {
		...
		async addReply() {
			...
			await this.$http.get(`${addCommentUrl}`,{
				tid:this.info.comment.tid,
				pid:this.replyPid,
				content: this.commentContent,
			})
			.then(async res => {
				if (res) {
					this.info.reply.list = [res,...this.info.reply.list];
					this.info.comment.replyNum=this.info.comment.replyNum+1
		...
		async getReply() {
				await this.$http.get(`${commentListUrl}`,{
					pid:this.info.comment.id
				})
					.then(async res => {
						console.log('res:',res);
						if (res) {
							this.info.reply = res;
							console.log('reply:',this.info.reply.list );
							// this.showPageLoading = false;
							// if (res.data.comment.page > this.page_index) {
							// 	this.hasMoreData = true;
							// } else {
