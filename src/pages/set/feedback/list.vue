		...
		<view class="rf-list" v-if="feedbackList.length > 0">
			...
			>
				<view class="wrapper">
					...
					<view class="address-box">
						<u-tag size="mini" plain shape="circle"  :text="item.type | feedbackFilter" />
					</view>
					<view class="u-box">
						<text class="name">{{ item.complaintsReplay || '管理员未回复' }}</text>
						<!-- <text class="mobile">联系方式：{{ item.username || '暂无' }}</text> -->
						<text class="mobile">提议时间：{{ time(item.createTime) || '暂无' }}</text>
					</view>
				</view>
			</view>
			<rf-load-more
				v-if="feedbackList.length > 0 && !loading" :status="loadingType"
			/>
		</view>
