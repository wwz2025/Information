			...
			>
				<swiper-item v-for="tabItem in tabBars" :key="tabItem.id">
					<scroll-view
						...
						<view v-for="(item, index) in tabItem.newsList" :key="index" class="news-item" @click="navToCommentClick(item)"  v-if="tabItem.newsId==2">
							<view class="comment-top" >
								...
								<view class="u-page__button-item" @click="addFocus(item)">
									<u-button
										:text="item.focusFlag==1?'已关注':'关注'"
										size="small"
										:type="item.focusFlag==1?'warning':'info'"
									></u-button>
								</view>
							</view>
						</view>

						<view v-for="(item, index) in tabItem.newsList" :key="index" class="news-item" @click="navToCommentClick(item)"  v-if="tabItem.newsId==1">
							<view class="comment-top" >
								<view class="comment-avatar">
									<u-avatar :src="item.avatar?fileUrl+item.avatar:headImg" mode="circle" size="30"></u-avatar>
									<view class="nick-name" style="margin: 8rpx 10rpx;">{{item.userName}}</view>
	...
	export default {
		...
		data() {
			return {
				...
				enableScroll: true,
				tabBars: [
					{id: "0",name: '粉丝',newsId: 1,region:'fans',tagId:'cms',newsList:[],refreshing:0,
						loadMoreStatus:0,  //加载更多 0加载前，1加载中，2没有更多了，3没有数据
					},
					{id: "1",name: '关注',newsId: 2,region:'focus',tagId:'cms',newsList:[],refreshing:0,
						loadMoreStatus:0,  //加载更多 0加载前，1加载中，2没有更多了，3没有数据
					},
		...
		methods: {
			...
			loadNewsList(type){
				...
				this.pulldownText="";
				this.$http.get(
					(tabItem.region=='fans'?`${fansListUrl}`:`${focusListUrl}`)+uni.getStorageSync('userInfo').userId, {
			...
			/*添加、取消关注*/
			addFocus(item){
				this.$http.get((item.isFocus?`${deleteFocusUrl}`:`${addFocusUrl}`)+item.userId)
					.then(async r => {
						if(r){
							...
							this.$mHelper.toast(item.isFocus?"已关注":"已取消");
							//触发刷新列表
							// uni.$emit('reloadData', {
							// 	msg: '刷新数据'
							// })
...
<style lang='scss'>
	...
	.u-page__button-item[data-v-62571508] {
