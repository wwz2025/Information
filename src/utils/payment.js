	...
	async weixinPay(order_group, data, route = '/pages/user/money/success', code) {
		...
		uni.login({
			...
			success: function (loginRes) {
				// http
				// 	.post(url)
				http.post(url+`?code=${loginRes.code}`, {
						// type: 1,
						payMethod: 1,
						orderSn:data.orderCode,
						goodsPrice:data.money,
						// order_group,
						// pay_type: 1,
					...
					.then(r => {
						uni.requestPayment({
							...
							success: () => {
								mHelper.toast('支付成功!');
								url=`${wechatnotify}`;//微信支付成功后回调
								http.post(url+`?code=${loginRes.code}`, {
										payMethod: 1,
										orderSn:data.orderCode,
										goodsPrice:data.money,
										confirmTime:data.inputAmountGive
									})
									.then(r => {
										mRouter.redirectTo({route});
									});
...
/*
	 ...
	 *@param data 订单 {“order_id”:199} 充值 {“money”:100};
	 */
	async weixinPay1(order_group, data, route = '/pages/user/money/success', code) {
		// #ifdef H5
	//	if (!this.isWechat()) {//TODO
